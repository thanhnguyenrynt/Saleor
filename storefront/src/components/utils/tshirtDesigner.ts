...
class TShirtDesigner {
  ...
  public async importDesignFromJson(jsonContent: string) {
    try {
      ...
      // Kiểm tra cấu trúc của object parsed để đảm bảo nó là DesignInfo
      ...
      if (
        typeof parsedJson.backgroundColor !== 'string' ||
        !parsedJson.frontDesign ||
        !parsedJson.backDesign ||
        !Array.isArray(parsedJson.frontDesign.images) ||
        !Array.isArray(parsedJson.frontDesign.texts) ||
        !Array.isArray(parsedJson.frontDesign.shapes) ||
        !Array.isArray(parsedJson.backDesign.images) ||
        !Array.isArray(parsedJson.backDesign.texts) ||
        !Array.isArray(parsedJson.backDesign.shapes)
      ) {
        throw new Error('Invalid design format');
      }

      const designInfo = parsedJson as DesignInfo;
